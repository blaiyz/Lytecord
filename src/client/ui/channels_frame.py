...
class ChannelsFrame(CTkFrame):
    def __init__(self, *args, channel_box: ChannelBox, client: Client, **kwargs):
        ...
        self._create_channel_button = CTkButton(self, text="Create Channel", image=icon, corner_radius=5,
                                                fg_color=FG_COLOR, hover_color="#727272",
                                                command=self.on_create_channel, font=font)
        ...
        self._guild_code_button = CTkButton(self._guild_code_frame, text="", image=icon, corner_radius=5,
                                            fg_color=FG_COLOR, hover_color="#727272", command=self.refresh_guild_code,
                                            width=20, height=20)
        self._guild_code_button.grid(row=0, column=1, sticky="ew", padx=5, pady=5)

        self._username_label = CTkLabel(self, text="Username", font=CTkFont(size=17, weight='bold'), fg_color=FG_COLOR,
    ...
    def on_create_channel(self):
        ...
        def callback(channel: Channel | None, message: str):
            if channel is not None:
                ...
                box = CTkMessagebox(title="Create Channel",
                                    message=f"Successfully created channel with name \"{channel.name}\"", icon="check",
                                    option_1="Ok")
                self.center_top_level(box)
                return

            box = CTkMessagebox(title="Create Channel", message="Error at creating channel: " + message, icon="cancel",
