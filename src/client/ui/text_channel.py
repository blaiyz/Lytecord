...
class TextChannel(CTkFrame):
    ...
    def __init__(self, *args, message_manager: MessageManager, client: Client, **kwargs):
        ...
        # Messages frame
        self._messages_frame = CTkScrollableFrame(self, label_anchor="n", label_text="",
                                                  label_font=CTkFont(family="Helvetica", size=20),
        ...
        self._entry = CTkEntry(self._entry_frame, fg_color="transparent", border_width=0,
                               font=CTkFont(family="Helvetica", size=18), corner_radius=5, validate="key",
                               validatecommand=(self.master.register(self._character_limit), '%d', '%P'))
        ...
        self._attachment_button = CTkButton(self._entry_frame, text="", image=attachment_icon, bg_color=ENTRY_COLOR,
                                            fg_color=ENTRY_COLOR, corner_radius=5, width=32, height=32,
                                            hover_color=HOVER_COLOR, command=self._attachment_button_click)
        ...
        self._attachment_frame_force_grid = CTkFrame(self._attachment_frame, fg_color='transparent', corner_radius=0,
                                                     width=0, height=0)
        ...
        if total < MAX_MESSAGES and self._top_row >= MIN_ROW and self._bottom_row <= MAX_ROW:
            if from_top:
                if not was_empty:
                    ...
                    rel_pos = (int(self._messages_frame._parent_canvas.yview()[
                                       0] * self._messages_frame.winfo_height()) - frame_pos, prev_top_frame)
                ...
                if not was_empty:
                    ...
                    rel_pos = (int(self._messages_frame._parent_canvas.yview()[
                                       1] * self._messages_frame.winfo_height()) - frame_pos, prev_bottom_frame)
    ...
    def _send_message(self, _: tk.Event | None = None):
        ...
        try:
            ...
            staged_message = Message(t << TAG_BYTE_LENGTH, self._channel.id, self._entry.get(), self._attachment,
                                     self._client.user, t)
        except ValueError:
        ...
        def attachment_upload_callback(attachment: Attachment | None, msg: str):
            ...
            if attachment is None:
                ...
                return
            message = Message(staged_message.id, staged_message.channel_id, staged_message.content, attachment,
                              staged_message.author, staged_message.timestamp)
    ...
    def _attachment_button_click(self):
        ...
        try:
            # Ask for a file
            f = filedialog.askopenfile(title="Select an attachment", filetypes=filetypes, mode="rb",
                                       initialdir="Pictures")
        ...
        try:
            attachment = Attachment(0, os.path.basename(f.name), AttachmentType.IMAGE, image.width, image.height,
                                    len(blob))
        ...
        self._attachment_image = LoadableImage(self._attachment_frame, image=image, corner_radius=5,
                                               client=self._client, max_width=THUMBNAIL_MAX_DIM[0],
                                               max_height=THUMBNAIL_MAX_DIM[1])
