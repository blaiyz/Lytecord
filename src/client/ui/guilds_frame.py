...
class GuildsFrame(CTkFrame):
    def __init__(self, *args, channels_frame: ChannelsFrame, client: Client, **kwargs):
        ...
        join_icon = CTkImage(light_image=JOIN_GUILD_ICON_LIGHT, dark_image=JOIN_GUILD_ICON_DARK,
                             size=(BUTTON_SIZE, BUTTON_SIZE))
        self._join_guild_button = CTkButton(self, text='', corner_radius=10, fg_color="transparent",
                                            hover_color="#858585", image=join_icon, command=self.on_join_guild,
                                            width=BUTTON_SIZE, height=BUTTON_SIZE)
        self._join_guild_button.grid(row=2, column=0, padx=10, pady=5)

        create_icon = CTkImage(light_image=CREATE_GUILD_ICON_LIGHT, dark_image=CREATE_GUILD_ICON_DARK,
                               size=(BUTTON_SIZE, BUTTON_SIZE))
        self._create_guild_button = CTkButton(self, text='', corner_radius=10, fg_color="transparent",
                                              hover_color="#858585", image=create_icon, command=self.on_create_guild,
                                              width=BUTTON_SIZE, height=BUTTON_SIZE)
    ...
    def on_join_guild(self):
        ...
        def callback(guild: Guild | None, message: str):
            if guild is not None:
                ...
                box = CTkMessagebox(title="Join Guild", message=f"Successfully joined guild with name \"{guild.name}\"",
                                    icon="check", option_1="Ok")
                self.center_top_level(box)
                return

            box = CTkMessagebox(title="Join Guild", message="Could not join the guild: " + message, icon="cancel",
                                option_1="Ok", option_2="Retry")
            self.center_top_level(box)
    ...
    def on_create_guild(self):
        ...
        def callback(guild: Guild | None, message: str):
            if guild is not None:
                self.add_guild(guild)
                box = CTkMessagebox(title="Create Guild",
                                    message=f"Successfully created guild with name \"{guild.name}\"", icon="check",
                                    option_1="Ok")
                self.center_top_level(box)
                return

            box = CTkMessagebox(title="Create Guild", message="Error at creating guild: " + message, icon="cancel",
                                option_1="Ok", option_2="Retry")
            self.center_top_level(box)
